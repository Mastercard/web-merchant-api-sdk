...
zappMerchantApi.getCfiDetails = new Promise(
    	...
    	try{
	        var bankJson = $.getJSON(cdnUrl)
				...
				.fail(function(jqObj, errTyp, errTxt ) {
						postError("S9998","An error has occurred while reading the JSON file. Error Type: " + errTyp + ". Error Text:"+ errTxt);
				});
    	}
    	catch(e){
    		postError('S9999',"An unknown error has occured");
    	}
    }
);
...
zappMerchantApi.invokeApp = function(cfiId,aptrId,productType){
	...
	try{
		if(typeof(cfiId) == "undefined" ){
			...
			appUrl = cfiMetaDataList[cfiId].cfiUniversalLink+'?aptrId='+aptrId+'&productType='+productType; 
